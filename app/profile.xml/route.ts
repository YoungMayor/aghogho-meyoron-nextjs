...
function serializeToXML(data: Record<string, unknown>): string {
  const buildXML = (obj: Record<string, unknown>): string => {
    ...
    for (const prop in obj) {
      ...
      const safeKey = prop.replace(/[^a-zA-Z0-9]/g, '_'); // XML tags cannot have spaces or weird chars usually
