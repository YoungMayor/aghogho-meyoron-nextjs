...
export default function ClassicTemplate({ data, config }: ClassicTemplateProps) {
  ...
  return (
    <div className="resume-classic bg-white dark:bg-white text-black p-8 print:p-6 shadow-lg print:shadow-none min-h-[297mm]">
      {/* Header */}
      <div className="resume-header mb-6 flex justify-between items-start gap-6">
        <div className="flex-1">
          <h1 className="text-3xl font-bold mb-2" style={{ color: config.themeColor }}>
            {profile.name}
          </h1>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-sm mb-2">
                {socialLinks.map((link) => (
                  <a
                    ...
                    style={{ color: config.themeColor }}
                  >
                    <Icon.fromIcon icon={link.icon} size={12} />
      ...
      <div className="resume-body space-y-6">
        {config.showSections.summary && (
          <section className="resume-section">
            <h2
              className="resume-section-title uppercase tracking-wide border-b mb-3 pb-1"
                ...
                <div key={item.company_name} className="resume-item">
                  <div className="flex justify-between items-start mb-1">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 font-medium">{item.company_name}</p>
                    </div>
                  ...
                  <p className="text-sm mb-2 text-gray-800">{item.description}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {config.showSkillIcons ? (
                      skill.icons.slice(0, 12).map((icon) => (
                        <span
                          key={icon.label}
                          className="flex gap-1 text-xs px-2 py-1 bg-gray-100 dark:bg-gray-100 rounded text-gray-800 border"
                        >
                  ...
                  {config.showSkillIcons && (
                    <div className="flex flex-wrap gap-1">
                      {project.icons.map((icon) => (
                        <span
                          key={icon.label}
                          className="flex gap-1 text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-100 rounded text-gray-800 border"
                        >
                          <Icon.fromIcon icon={icon} size={12} />
          ...
          <section className="resume-section">
            ...
            <div className="flex flex-wrap gap-2">
              {badges.map((badge) => (
                <span
                  key={badge}
                  className="text-xs px-3 py-1 bg-gray-100 dark:bg-gray-100 rounded-full text-gray-800 border"
                >
                  {badge}
                </span>
              ))}
