import { Profile } from '@/lib/types';
import { gravatar } from '../utils/helpers';

export const profile: Profile = {
  name: 'Aghogho Meyoron',
  titles: ['Software Engineer', 'Tech Mentor', 'Technical Writer'],
  avatar_url: await gravatar('youngmayor.dev@gmail.com'),
  biography: `<div>I am <b class='text-primary'>Aghogho Meyoron</b>, a Software Architect focused on high-integrity systems. I bridge the gap between complex backend engineering and seamless user experience, specializing in fintech infrastructure, AI-driven automation, and developer tooling. My work is defined by architectural rigour, automated testing, and a relentless drive for system efficiency.</div>`,
  profile_link: null,
  notes: {
    tagline: 'Architecting resilient systems and the tools that build them.',
    persona: 'Problem Solver, Gamer and Swimmer.',
    about:
      'Senior Software Engineer with 6+ years of experience building and scaling fintech and business-critical web platforms. Specialised in backend architecture, secure payment systems, platform migrations, and high-performance API design using Laravel, Node.js, and modern cloud tooling. Proven track record delivering fraud prevention systems, rewards engines, internal developer tools, and AI-powered products. Also active as a technical writer and mentor, translating complex engineering concepts into practical guidance for developers.',
  },
  contact: {
    email: 'youngmayor.dev@gmail.com',
    message: 'Lets work together!',
  },
  copyright: {
    year: '2025',
    creator_name: 'MayR Labs',
    creator_link: 'https://mayrlabs.com',
  },
};
